...
df['bid_depth_size_cumu'] = df['bid_price'].apply(lambda x: x[0][1] if x else None).cumsum()
df['ask_depth_size_cumu'] = df['ask_price'].apply(lambda x: x[0][1] if x else None).cumsum()
...
df['order_book_imbalance'] = (df['bid_depth_size_cumu'] - df['ask_depth_size_cumu']) / (df['bid_depth_size_cumu'] + df['ask_depth_size_cumu'])
